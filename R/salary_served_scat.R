...
p = ggplot(mydata, aes(x=Y6_Served, y=Y6_Max_Salary)) + 
  geom_point(color='#999999', alpha=0.5)  + 
  geom_smooth(method=lm, se=FALSE, color='#777777') + 
  labs(x='Years Served', y='Max Salary Achieved') + 
  scale_y_continuous(breaks=seq(0,200000,50000), labels=c('$0','$50k','$100k', '$150k','$200k'), 
                     limits=c(0,200000)) + scale_x_discrete(breaks=seq(1,6)) + 
  ggtitle('Max Salary Achieved vs. Years Served') + 
  theme_minimal()
